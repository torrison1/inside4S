...
<script>
	...
	$(function(){
		...
		$('.change_pass').on('click', function(){
			...
			var options = {
				url: "<?=$lang_link_prefix?>/auth/api/change_password/",
		...
		$('.update_info').on('click', function(){
			...
			var options = {
				url: "<?=$lang_link_prefix?>/auth/api/update_user_data/",
				success: function(obj) {
					if (obj.status == "success") {
						...
						// document.location.href='<?=$lang_link_prefix?>/auth/page/profile/';
						// Reload avatar?
			...
			$.confirm({
				...
				buttons: {
					confirm: {
						...
						btnClass: 'btn-blue',
						action: function () {
							$.post('/auth/api/phone_verification', {},
								function (data) {
			...
			$.confirm({
				...
				buttons: {
					confirm: {
						text: 'Да',
						action: function () {
							$.post('/auth/api/email_verification', {},
								function () {
									$.alert({
										type: 'green',
										title: 'Верификация',
					...
					formSubmit: {
						...
						action: function () {
							...
							if(code.length != 5){
								...
								$.post('/auth/api/check_phone_verification_code', {code: code},
									function (data) {
										var info = jQuery.parseJSON(data);
										if(info.status == 'error') {
											...
											setTimeout(function(){
												window.location.href = '/auth/page/profile';
											},1000);
										}
									}
								);
