...
Class InsideModularSystem
{
    ...
    public function check_update_modules_files_relations()
    {
        ...
        $obj = new \Tools\InsideTools\InsideProjectFiles;
        $obj->view();
    ...
    public function check_update_modules_database_relations()
    {
        ...
        $obj = new \Tools\InsideTools\InsideDatabaseView;
        $obj->db =& $this->db;
        $obj->view();
        ...
        if (isset ($this->data['module_arr']['files_json']) AND ($this->data['module_arr']['files_json'] != '')) {
            ...
            foreach ($module['files'] as $file) { ?>
                ...
                <div style="border: 1px solid silver; padding: 10px; margin-top: 5px; width: 60%">
                    ...
                    // TMP fixes // TODO Modular system Needs Refactoring!
                    $file['path'] = str_replace('/Inside4/CommonCore/Globals','/Tools/CommonCore/Debugger', $file['path']);
                    $file['path'] = str_replace('/Inside4/','/Tools/', $file['path']);
                    $file['path'] = str_replace('/AppViews/','/Views/', $file['path']);
                    $file['path'] = str_replace('/AppControllers/','/Services/', $file['path']);
