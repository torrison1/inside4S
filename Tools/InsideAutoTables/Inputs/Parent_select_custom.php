...
class Parent_select_custom {
    ...
    public function input_form($input_array)
    {
        ...
        $menu_res = $db->sql_get_data("SELECT ".$input_array['select_index']." AS id, ".$input_array['select_field'].", ".$input_array['select_pid_index']." AS parent_id FROM ".$input_array['select_table']." ".$input_array['select_field']." ".$input_array['rules'] );
        return "
		<script>
  		$(function() {
    ...
    {
        $data = '';
        $data .= '<select id="'.$select_name.'" class="'.$select_name.' input form-control selectpicker" name="'.$select_name.'" style="width: 350px;">';
