...
Class Info_system
{
    ...
    public function pages_list($page = '1', $per_page = 1, $user_owner_id = false, $filter_type = false, $filter_data = false)
    {
        ...
        if ($GLOBALS['app']['translate']['active_language'] == 'en')
        {
        ...
        else
        {
            $res = $this->db->sql_get_data("SELECT
										...
										AND it_content_translate.content_lang_alias = ".$this->db->quote($GLOBALS['app']['translate']['active_language'])."
										{$filter_join}
										WHERE
    ...
    public function get_all_categories_arr($only_top = true, $pid = false)
    {
        ...
        // \Tools\CommonCore\Debugger::debug_global(true);
        ...
        else
        {
            $res = $this->db->sql_get_data("SELECT 
										...
										AND it_categories_translate.categories_lang_alias = ".$this->db->quote($GLOBALS['app']['translate']['active_language'])."
